...
assert.equal(index.stats.conceptCount, 568);
...
assert.equal(index.entries.filter((entry) => entry.commonMisreadings.length).length, 568);
...
// and pressure test 07 added the attribution fork and the ambiguity tax,
// likewise with both.
assert.equal(index.entries.filter((entry) => entry.boundaryConditions.length).length, 535);
