...
assert.equal(index.stats.conceptCount, 569);
...
assert.equal(index.entries.filter((entry) => entry.commonMisreadings.length).length, 569);
...
// distance discount, likewise with both.
assert.equal(index.entries.filter((entry) => entry.boundaryConditions.length).length, 536);
