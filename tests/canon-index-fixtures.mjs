...
assert.equal(index.stats.conceptCount, 715);
assert.equal(index.stats.sourceCount, 22);
...
assert.equal(index.entries.filter((entry) => entry.commonMisreadings.length).length, 715);
...
// and pressure test 07 added the attribution fork, the ambiguity tax and the
...
// boundary gap remains 33. Deep Dive essay 08 (autism and dating) added a source page
// and eight concepts — hub, root and six sections — each authored with both fields, so
// the gap is still 33.
assert.equal(index.entries.filter((entry) => entry.boundaryConditions.length).length, 682);
...
assert.equal(required('frameworks:repair-sequence').sourceLinks.length, 3);
assert(required('frameworks:repair-sequence').aliases.includes('relationship repair sequence'));
assert(required('frameworks:repair-sequence').phrases.includes('forgiveness does not require reconciliation'));
assert(required('frameworks:repair-sequence').related.includes('frameworks:agreement-surface'));
...
assert.equal(required('frameworks:sleep-interaction-loop').sourceLinks.length, 3);
assert(required('frameworks:sleep-interaction-loop').aliases.includes('dyadic sleep and relationship functioning'));
assert(required('frameworks:sleep-interaction-loop').phrases.includes('total sleep deprivation before couple conflict'));
assert(required('frameworks:sleep-interaction-loop').related.includes('frameworks:stress-transmission-split'));
assert(required('frameworks:stress-transmission-split').related.includes('frameworks:sleep-interaction-loop'));
assert(required('frameworks:satisfaction-flywheel').related.includes('frameworks:sleep-interaction-loop'));
assert(required('frameworks:repair-sequence').related.includes('frameworks:sleep-interaction-loop'));
