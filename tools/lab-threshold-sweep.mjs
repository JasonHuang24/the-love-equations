  ...
  if (options.neighbors) {
    ...
    fs.writeFileSync(options.neighbors, `${JSON.stringify({
      ...
      note: 'Frozen threshold-neighbour band: every corpus pair within ±band of an'
        + ' admission line. `scores` pins which SIDE of each line a pair sits on, so a'
        + ' change that flips one fails the suite even if nobody captured a baseline —'
        + ' which is what this is FOR. It is the second line of defence, not the primary'
        + ' one: measured over every crossing ever recorded, the band alone would have'
