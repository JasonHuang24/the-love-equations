...
Reason precedence, corrected in the v2.7.3 review to state the order
`reasonFor()` actually implements (review finding F-12): outside the human
frame (technical vocabulary first, then the adjacent-frame check), descriptive
evidence, extraction furniture, boundary or moderator evidence,
existing-doctrine retrieval miss, then insufficient evidence with possible
doctrine gap as the fallthrough. Extraction furniture resolves to **insufficient
evidence**, not to the outside-human reason; and it is tested AFTER descriptive
evidence on purpose, so statistical table language keeps the descriptive-fact
reason — `tests/lab-unmatched-umbrellas.test.mjs` pins that case. Boundary
evidence remains boundary evidence even when the umbrella has a current
doctrine owner.
...
**Scope of the correction, restated after the v2.7.3 review (finding F-8).**
The paragraph above named one correction; `caseCorrections` carries eight, and
five of them are expected-value changes rather than the documented `brief-03`
kind fix:

| Case | Correction | Character |
|---|---|---|
| `brief-03` | `kind` negative-control → positive | the documented contradiction |
| `asym-01`, `asym-02`, `authority-01`, `authority-02`, `authority-06` | expected `unmatchedReasonId` → `existing-doctrine-retrieval-miss` | follows the owner-metadata behaviour introduced in the same session |
| `access-05`, `access-06` | expected `secondaryUmbrellaId` → `null` | the 1.1 explicit-mechanism standard, recorded in the fixture as `standardReconciliation` |

Nothing was hidden — every correction is enumerated in the fixture with written
rationale and the 1.0 bytes are untouched — but the headline "unmatched-reason
agreement 61/61 (100%)" is measured against expectations that were adjusted for
five cases to match behaviour shipped in the same change, and the 1.0 fixture's
own `mutationPolicy` says not to tune cases after implementation begins. Those
five are therefore NOT pre-registered evidence. They are recorded here so a
future reader does not read 100% as an untouched result, and they are left as
Jason's ruling rather than reopened by the reviewer.
...
## v2.7.3 — independent review and integration (2026-08-31)

An independent review of `97f766a` ran the shipped build against a fresh
43-source window (251,030 words, 1,060 unmatched items) and a rendered browser
pass that the original run had recorded as blocked. It confirmed the
architecture — one production call path, post-disposition only, exact fragment
only, and a 29-source invariance projection that catches every mutation its
self-test throws at it — and found the rule layer doing surface-token matching
where this record promised mechanism matching.

Six mechanisms produced classifications a reader could see. Thirty of
thirty-five constructed probes failed their stated requirement and nineteen of
those reached the live unmatched queue. All are fixed in v2.7.3; the guards are
committed in `tests/lab-unmatched-umbrellas.test.mjs` and every one of them was
red on v2.7.2 before its rule was written.

| Finding | Defect | Rule now |
|---|---|---|
| F-1 | `split` matched the bare token `separate`, so separate records, consent forms, columns, rooms, databases, fees and appointments classified as Role unbundling — six at 0.99, most of them live | a separation verb must govern a role noun, state the separation outright, or carry a `from`-frame naming a partner or parent |
| F-2 | bare `recognize` plus bare `benefit` cleared External recognition at 0.99 with reason "Possible doctrine gap" on sentences about mutual benefit and sharing housework | the verb counts only when an institutional actor or a status object governs it |
| F-3 | the focus ring on every unmatched card was clipped away: `css/lab.css:36` scores 0-3-0 and beat the intended inset rule at 0-2-1, restoring an outward offset against `overflow: hidden` | the card rule is qualified with `.lab-page` and outranks it |
| F-4 | `api`, `containers` and `customer support` vetoed genuine relational prose and asserted "Outside the human-relational frame" about Replika and supervisory-romance sentences | the three ambiguous tokens are dropped; the multiword technical terms hold all eight negative-control sources |
| F-5 | fifteen corpus literals (`university policy`, `the relationship between`, `details of`, …) forced abstention on ordinary prose — one word changed flipped a 0.68 classification to an abstention | replaced by a length-and-predication heading test that generalises off-corpus |
| F-6 | `students` in the authority family read a research sample as institutional governance, asserting retrieval-miss against The Authority Firewall on a narcissism title | the token stays (policies need it); research-population frames are guarded in `isTriageFurniture` instead |
| F-7 | methods sentences naming their instrument rather than their participants scored on their own subject matter | coding and annotation procedure is tested by its verb, wherever it sits |
| F-10 | `suggestedDestination` was dropped from the research-queue Markdown for every triaged item while legacy items kept it | restated as its own line in the item body |
| F-12 | this record's precedence list did not match `reasonFor()` | corrected above; the code was right |
| F-13 | only `:` and `?` endings were read as headings, so a title ending in a period classified at 0.66 | verbless-title test, calibrated against the frozen set |
| F-14 | this record was a new `md/*.md` file with no INDEX row | folded here; INDEX row added |

**Not integrated, and why.** F-9 — a live `git worktree` is still registered at
`artifacts/worktrees/unmatched-umbrellas` on branch `codex/unmatched-umbrellas`,
which `CLAUDE.md` prohibits by name. The hazard is contained (its `lab-corpus/`
is a real 59-file copy, not a junction — verified), but the removal command is
the one that once destroyed the archive, so it is left for Jason rather than run
by an agent. F-11 — a speaker's job title inside a fragment can manufacture a
secondary umbrella; the classifier is behaving to contract (it receives the
exact fragment) and the fix belongs in extraction, not here.

**Calibration that is load-bearing.** Capitalization is deliberately not
consulted by the heading test. Title case is the obvious signal and it is
unusable: the audits reclassify every fragment upper-cased to prove case is
never load-bearing, and a title-case test answers differently for a title and
its upper-cased twin. The first attempt at F-13 used it and `access-11` broke
under normalization — the audit caught it. Length and predication survive that
normalization. The 8-word cap is measured, not chosen: the shortest fragment the
frozen evaluation expects to be SUPPORTED is 10 words, and the titles run 5 to 8.

**Measured after the change.** Frozen evaluation 61/61 with the fixtures
untouched; adversarial fixture 20/20; the 30 manually-verified-correct live
classifications from the review window all retained; all four of its
manually-verified-wrong ones no longer asserted; 61/61 mechanism probes pass.
Fresh-window precision on supported classifications moved from 30/34 (88.2%) to
30/30 (100%): the four wrong assignments now abstain rather than being
reclassified into a different umbrella, and no correct one was lost. Mapped,
unmatched and excluded totals are byte-identical to the pre-change run
(128 / 1,060 / 26,373), which is the same thing the invariance audit proves at
the projection level. Abstention moved 96.79% → 97.17%.

**Still open, unchanged by this pass.** Role and Brief produced no assignments in
the fresh window and External recognition produced three; recall for those three
