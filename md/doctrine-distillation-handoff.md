...
4. MANDATORY CHECK before calling anything a gap: verify against the LIVE SITE, not the
   Lab verdict alone. Harvest #1's hard lesson (md/doctrine-backlog-harvest-01.md): an
   unmapped verdict can mean the canon index under-represents an existing page — the fix
   is overlay enrichment (data/canon-overlay.json), not new doctrine. Use an Opus verifier
   against the actual pages (statistics, frameworks, mythbuster, pills, lexicon, deep dives).
...
candidate core themes ranked by (evidence strength x distinctness from canon), each with
the live-site check result, and a dead-candidates section recording what you checked and
killed (so the other lane doesn't rediscover it). Commit the dossier when it's verified.
